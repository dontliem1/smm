...
(function () {
    ...
    var keys = (shower) => {
        ...
        shower.container.addEventListener('touchstart', (event) => {
            if (event.changedTouches[0].clientX < 100) {
                event.preventDefault();
                shower.prev();
            } else if ((window.innerWidth - event.changedTouches[0].clientX) < 100) {
                event.preventDefault();
                shower.next();
            } else if (event.changedTouches[0].clientY < 75) {
                event.preventDefault();
                shower.exitFullMode();
            }
        });
